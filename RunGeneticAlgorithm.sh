...
if (( $r == 1 ))
  ...
  $JULIA --threads 12 $JPATH/makeInitialGeneration.jl
  $JULIA --threads 12 $JPATH/RunMassModel.jl
  ...
  let j=$j+1
  $JULIA --threads 12 $JPATH/makeNextGeneration.jl
  $JULIA --threads 12 $JPATH/RunMassModel.jl
  matlab -batch "try;run('$MPATH/Run');catch;end;exit"
  done

  $JULIA --threads 12 $JPATH/GetFitnessLastGen.jl
