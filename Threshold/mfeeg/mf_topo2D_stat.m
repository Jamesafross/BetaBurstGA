function Zi = mf_topo2D(ch_pos,ch_plot,ch_display,Values,is_display_ch,surface_lim,criterion_stat_value)
% mf_topo2D		plot topography in 2D.
% Usage
%	mf_topo2D(ch_pos,ch_plot,ch_display,Values,is_display_ch,,surface_lim,criterion_stat_value)
% Input
...
%				a time band and a frequency band. here use stat value, e.g., t value.
...
%   criterion_stat_value -- e.g.. 14 sbj, t=2.145 p=0.05 (please check stat value tables). input 2.145, p<0.05 will be displayed as black.
...
%		1 mf_topo2D(ch64_pos_2D,[1:64],[1:64],value,1,[],2.145); 
%			% calulate all ch, and display ch on the map.
%		2 mf_topo2D(ch64_pos_2D,[1:64],[1:64],value,0,[],2.145); 
%			% calulate all ch, not display ch. (but ch_display still needed to be input. )
%		3 mf_topo2D(ch64_pos_2D,[1:30,33:64],[1:30,33;64],value,1,[],2.145); 
%			% exclude eog in calulating, and exclude eog in displaying ch.	
%		4 mf_topo2D(ch64_pos_2D,[1:9,11:30,33:64],[1:30,33;64],value,1,[],2.145); 
%			% exclude ch 10 in calulating(bad ch), but still display it.
%		5 mf_topo2D(ch64_pos_2D,[1:9,11:30,33:64],[1:9,11:30,33;64],value,1,[],2.145); 
%			% exclude ch 10 in calulating(bad ch), and not display it.
%		6 mf_topo2D(ch64_pos_2D,[1:9,11:30,33:64],[1:9,11:30,33:64],value,1,15,2.145); 
%			% input the most value 15 as the scale of map.
%		7 mf_topo2D(ch64_pos_2D,[1:30,33:64],[1:30,33;64],zeros(1,64),1,[],2.145); 
...
non_sig=find(Zi>-criterion_stat_value & Zi<criterion_stat_value);
Zi(non_sig)=0;
%%%%% Plot topography
% plot values
...
if is_display_ch == 1 
	plot(x_display,y_display,'.','Color',[1 1 1],'markersize',8); % why this influence color when 32 ch?
%{	
	for i = 1:length(x_display)
