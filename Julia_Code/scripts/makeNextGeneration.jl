...
joinpath("$DATADIR","Generation.jl")
gen_phenotype = load(joinpath("$DATADIR","Generation.jl"), "GenPop")
...
elseif configuration["type_burst_analysis"] == "HMM"
    file = matopen(joinpath("$DATADIR","HMMStats.mat"))
    phenotype_stats = read(file, "stats")
    close(file)

    include(joinpath("$DATADIR","HMMrealDataStats.jl"))
end

best_phenotype = load(joinpath("$DATADIR","BestInGen.jld"), "BestPhenotype")
...
open(joinpath("$DATADIR","BestFitnessLog.txt"), "w") do f #outputs best fitness phenotype to a .txt file
end



next_gen = make_next_gen(winners_tour, best_in_gen, size_pop, num_child, const_params)
save(joinpath("$DATADIR","LastGeneration.jld"), "GenPop", gen_phenotype)
save(joinpath("$DATADIR","Generation.jld"), "GenPop", next_gen)

@testset "non-zeros phenotypes" begin
end


last_generation_data = JSON.parsefile(joinpath("$PROGDIR","generation_results.json"))
