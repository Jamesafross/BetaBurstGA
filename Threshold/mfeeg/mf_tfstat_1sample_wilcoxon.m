function [pval,statval] = mf_tfstat_1sample_wilcoxon(tf_group)
% mf_tfstat_1sample_wilcoxon	statis  data using one sample wilcoxon.
%	[pval,statval] = mf_tfstat_1sample_wilcoxon(tf_group)
% Output
...
%-------------------------------------------------------------------------
if nargin~=1
    disp('mf_tfstat_1sample_wilcoxon requires 1 input arguments!')
	return
end
...
for i = 1:o
    ...
    for j = 1:m
        ...
        for k = 1:n
            ...
            [pval(j,k,i),h,stats] = signrank(vect1);
            if mean(vect1) < 0
